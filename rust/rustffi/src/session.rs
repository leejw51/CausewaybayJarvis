...
use crate::abi::{
    drop_handle, event_bridge, opt_str_arg, out_json, out_string, str_arg, Busy, Handle,
    JarvisEventFn, JarvisParams,
};
...
impl Handle for JarvisSession {
    const TAG: u64 = 0x4a5f_7365_7373_5f31; // "J_sess_1"
}
...
///
/// A session cannot be closed from inside its own event callback: the turn
/// still running on it holds the object. Such a call is refused, leaving the
/// session open, and the reason lands in `jarvis_last_error`.
///
/// # Safety
...
///
/// The callback must not call back into this library on the same session: the
/// turn holds the session for its whole duration. A reentrant call is refused
/// with an error rather than allowed to corrupt it. Everything the callback is
/// likely to want — the tokens, the prefill counts, the interrupt flag — is
/// either handed to it or reachable without the session handle.
///
/// # Safety
...
pub unsafe extern "C" fn jarvis_send(
    ...
    guard(-1, || {
        ...
        // Refuse a call that re-enters on this session while the turn below is
        // handing control back to the caller's language for every token.
        let _busy = Busy::claim(&*session);
